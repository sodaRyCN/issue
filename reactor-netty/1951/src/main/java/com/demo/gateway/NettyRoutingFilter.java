...
/**
 * 在默认NettyRoutingFilter的基础上，设置每一笔请求上游的超时时间
 * chain的最后一个filter
 */
public class NettyRoutingFilter implements GlobalFilter, Ordered {

    private static final Logger log = LoggerFactory.getLogger(NettyRoutingFilter.class);
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        Flux<HttpClientResponse> responseFlux = this.httpClient.headers(headers -> {
            ...
            if (log.isDebugEnabled()) {
                nettyOutbound.withConnection(connection -> log.debug("outbound route: "
                        + connection.channel().id().asShortText() + ", inbound: " + exchange.getLogPrefix()));
            }
